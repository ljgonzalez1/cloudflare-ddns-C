...
int main(int argc, char **argv) {
  if (argc < 2) {
    fprintf(stderr, "Uso: %s <https_url> [-H \"Header: valor\" ...] [-d \"{...json...}\"]\n", argv[0]);
    return 1;
  }
  ...
  // Incluir: método, path, versión, Host, headers user-supplied, Content-Length, Connection
  char request[4096];
  int offset = 0;
  ...
  // Siempre añadir Content-Length
  offset += snprintf(request + offset, sizeof(request) - offset,
                     "Content-Length: %zu\r\n", data ? data_len : 0);

  // Añadir Connection: close
  offset += snprintf(request + offset, sizeof(request) - offset,
                     "Connection: close\r\n\r\n");
