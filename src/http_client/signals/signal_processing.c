...
static void signal_handler(int signum) {
    ...
    switch (signum) {
        ...
        case SIGINT:
            DEBUG_LOG("Received SIGINT (Ctrl+C), initiating cleanup...");
            break;

        case SIGTERM:
            DEBUG_LOG("Received SIGTERM, initiating cleanup...");
            break;

        case SIGHUP:
            DEBUG_LOG("Received SIGHUP, initiating cleanup...");
            break;

        case SIGQUIT:
            DEBUG_LOG("Received SIGQUIT, initiating cleanup...");
            break;

        default:
            DEBUG_LOG("Received signal %d, initiating cleanup...", signum);
            break;
    }
    ...
    DEBUG_LOG("Signal cleanup completed, exiting...");
...
int signals_init(void) {
    ...
    DEBUG_LOG("Initializing signal handling...");
    ...
    DEBUG_LOG("Signal handling initialized");
...
void signals_cleanup(void) {
    ...
    DEBUG_LOG("Cleaning up signal handling...");
    ...
    DEBUG_LOG("Signal handling cleanup completed");
}
...
int signals_register_cleanup(signal_cleanup_callback_t callback) {
    ...
    DEBUG_TRACE("Registered cleanup callback %d", signals_state.callback_count - 1);
